---
name: bdr-outreach-composer
description: Create and persist the BDR Outreach Composer lead brief plus three ranked full cold-email body drafts while using the current bdr_agent.stages.outreach_composer runtime and persisted lead_brief storage contract.
---
# BDR Outreach Composer
## Purpose
Use this canonical skill for the Outreach Composer stage of the BDR agent pipeline. The human-facing stage is Outreach Composer; the persisted runtime module remains `bdr_agent.stages.outreach_composer`, compact prompts use `BDR_AGENT_STAGE=lead_brief`, and the existing CLI packet shape is unchanged.
...
Do not ask agent_orchestrator to include company details, full research JSON, synthesis content, style snapshots, giant prompt variables, or draft copy in the agent prompt. BigQuery is the source of truth.
## Runtime preflight
Run from the BDR Agent repository root and use the prepared Python environment instead of the system `python`. Start each agent run with:
```bash
...
PY
```
If you use `bq`, query only the needed row. Do not paste large research JSON back into the agent prompt.
Read exactly these two durable editable references before drafting:
- `references/outreach_positioning_guide.md`: the positioning guide for frame, capability, buyer-problem, product-surface, and source-signal selection.
- `references/outreach_style_guide.md`: the BDR style guide for tone, opener shape, evidence handling, BDR-specific preferences when available, and bad-vs-better examples.
...
- Which style-guide principle applies from `references/outreach_style_guide.md`, including any owner-specific style when available.
- Which positioning-guide frame, buyer problem, concrete product capability, and product surface applies from `references/outreach_positioning_guide.md`.
...
These are email bodies, not intro hooks. The HubSpot property remains `ai_hook_intro` because that is the current production field, but the selected rank-1 content will be the full email body excluding the greeting line, sign-off line, and sender name because the HubSpot template supplies those.
...
- Give each draft a concrete product bridge. Choose one mechanism or surface from `references/outreach_positioning_guide.md` when useful. Keep it conversational and do not cram multiple surfaces into one body.
...
Before drafting, select exactly one primary positioning frame from `references/outreach_positioning_guide.md` and one applicable style-guide principle from `references/outreach_style_guide.md`. Use the references to choose the frame and borrow the level of specificity and plainspoken language, but do not paste guide, deck, style-guide, or internal GTM phrasing verbatim into final email copy.
...
- The product bridge is concrete enough to explain [your product] through at least one mechanism, buyer problem, or product surface from `references/outreach_positioning_guide.md`.
- The tone follows the applicable principle from `references/outreach_style_guide.md` and sounds like a human BDR note rather than a positioning paragraph.
