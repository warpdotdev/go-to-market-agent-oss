...
description: Create and persist the BDR Outreach Composer lead brief plus three ranked full cold-email body drafts while using the current bdr_agent.stages.outreach_composer runtime and lead_brief compatibility contract.
---
# BDR Outreach Composer
## Purpose
Use this canonical skill for the Outreach Composer stage of the BDR agent pipeline. Until the runtime-stage aliases are integrated, the compatibility/runtime module remains `bdr_agent.stages.outreach_composer`, compact prompts still use `BDR_AGENT_STAGE=lead_brief`, and the existing CLI packet shape is unchanged.
